...
def get_gps_stats(data, tf):
    ...
    try:
        ...
        # Properly format the timezone offset (e.g. -08:00)
...
def main():
    ...
    # --- 3. ATOMIC DRIFT CALCULATION (Stale GPS Filter) ---
    all_drifts = []
    all_offsets = []
    ...
    if not all_drifts:
        print("CRITICAL ERROR: No photos with GPS data found. Cannot establish timeline or timezone.")
        sys.exit(1)
    ...
    # Determine True Atomic Drift (Filtering out stale GPS fixes by looking for the maximum fresh modes)
    drift_counts = Counter(all_drifts)
    max_frequency = max(drift_counts.values())

    if max_frequency >= 2:
        # If there are bursts, the freshest live tracking will be the highest value among the most frequent
        most_common_drifts = [d for d, c in drift_counts.items() if c == max_frequency]
        global_drift = max(most_common_drifts)
    else:
        # If every fix is unique, the absolute freshest fix has the highest mathematical drift value
    ...
    for data in raw_data:
        ...
        frame_type = f"KEY FRAME ({consensus_tz})" if has_gps else "ORPHAN FRAME"
        ...
        # If the file has no offset, assume it needs the full timezone jump
        if existing_offset_sec is None:
            existing_offset_sec = target_offset_sec

        # Total Shift = (Difference in Timezones) + (The Atomic Clock Drift)
        tz_shift_sec = target_offset_sec - existing_offset_sec
        total_shift_sec = tz_shift_sec + global_drift

        # Idempotency check: If shift is 0 and timezone tag is perfect
